...
- Immutable hierarchy of values (tree) constructed from JSON data stream,<br>
  {@link fr.umlv.jsonapi.builder.ObjectBuilder} and {@link fr.umlv.jsonapi.builder.ArrayBuilder}
...
- High-level API to decode to a tree of Java objects from a JSON derived from the
  ...
  and its stream version {@link fr.umlv.jsonapi.bind.Binder#stream(java.io.Reader, Class)}.
